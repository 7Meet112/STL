...
6.front 		得到数组头的引用
7.back 			得到数组的最后一个单元的引用 int _it = vec.back(); std::cout << _it << std::endl;
8.max_size 		得到vector最大可以是多大
9.capacity 		当前vector分配的大小
10.size 		当前使用数据的大小
11.resize 		改变当前使用数据的大小，如果它比当前使用的大，则填充默认值
12.reserve 		改变当前vecotr所分配空间的大小
13.erase 		删除指针指向的数据项 返回的是下一个有效迭代器的值，可以通过iter = v.erase(iter)来避免迭代器失效。
14.clear 		清空当前的vector
15.rbegin 		将vector反转后的开始指针返回(其实就是原来的end-1)
16.rend 		将vector反转后的结束指针返回(其实就是原来的begin-1)
17.empty 		判断vector是否为空
18.swap 		与另一个vector交换数据
19.insert		插入，insert返回的是插入元素的迭代器的值，可以通过+2来避免迭代器失效。
```
###### 2.2 构造方法
```
...
v1.assign(2, 112); // 改变size和capacity:2//1234567 size=7 之后 112 112 size=2
